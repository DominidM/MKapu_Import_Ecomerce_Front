...
// Cast a any[] para evitar conflictos con campos opcionales del JSON
const productsData = rawProducts as any[];

const ALL_CATS = Array.from(new Set(productsData.map((p) => p.category as string)));

const allPrices = productsData
  .map((p) => p.price as number | undefined)
  .filter((p): p is number => typeof p === "number" && p > 0);

const PRICE_MAX = allPrices.length > 0
  ? Math.ceil(Math.max(...allPrices) / 100) * 100
  : 5000;
...
export default function ProductosPage() {
  ...
  return (
    <div className="productos-page">
      ...
      <div className="productos-layout">
        ...
        <aside className={`sidebar${sidebarOpen ? " sidebar--open" : ""}`}>
          ...
          <div className="sidebar__section">
            <label className="sidebar__label">Buscar</label>
            <input
              type="search"
              placeholder="Ej: máquina de hielo..."
              value={search}
            ...
            <div className="sidebar__cats">
              {ALL_CATS.map((cat) => (
                <label key={cat} className="sidebar__check-row">
                  ...
                  />
                  <span className="sidebar__check-label">{cat.replace(/-/g, " ")}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="sidebar__section">
            <label className="sidebar__label">
              ...
              <span className="sidebar__price-val">
                {maxPrice >= PRICE_MAX ? "Sin límite" : `S/ ${maxPrice.toLocaleString()}`}
              </span>
            </label>
            <input
              type="range"
        ...
        <main className="productos-main">
          <div className="productos-main__top">
            <p className="productos-main__count">
              ...
              {cats.length > 0 && ` en ${cats.map((c) => c.replace(/-/g, " ")).join(", ")}`}
            </p>
            {activeFilters > 0 && (
      ...
      <style jsx>{`
        .productos-page { max-width:1300px; margin:0 auto; padding:1.5rem 1rem 4rem; }

        .filter-toggle { display:none; align-items:center; gap:8px; background:#fff; border:1.5px solid #e0d8d0; border-radius:10px; padding:0.5rem 1rem; font-size:0.88rem; font-weight:600; cursor:pointer; margin-bottom:1rem; position:relative; color:#1a1a1a; transition:border-color 0.15s; }
        .filter-toggle:hover { border-color:#e05c2a; color:#e05c2a; }
        .filter-toggle__badge { background:#e05c2a; color:#fff; font-size:0.65rem; font-weight:700; border-radius:99px; padding:1px 6px; min-width:18px; text-align:center; }

        .productos-layout { display:grid; grid-template-columns:240px 1fr; gap:2rem; align-items:start; }

        .sidebar { background:#fff; border:1px solid #ede8e1; border-radius:16px; padding:1.25rem; position:sticky; top:80px; }
        .sidebar__header { display:flex; align-items:center; justify-content:space-between; margin-bottom:1.25rem; }
        ...
        .sidebar__label { display:flex; align-items:center; justify-content:space-between; font-size:0.75rem; font-weight:700; text-transform:uppercase; letter-spacing:0.07em; color:#888; margin-bottom:0.75rem; }
        .sidebar__price-val { font-size:0.8rem; font-weight:700; color:#e05c2a; text-transform:none; letter-spacing:0; }

        .sidebar__search { width:100%; box-sizing:border-box; padding:0.55rem 0.75rem; border:1.5px solid #e0d8d0; border-radius:10px; font-size:0.85rem; outline:none; transition:border-color 0.15s; background:#faf8f5; }
        .sidebar__search:focus { border-color:#e05c2a; }
        ...
        .productos-main__top { display:flex; align-items:center; justify-content:space-between; margin-bottom:1.25rem; flex-wrap:wrap; gap:8px; }
        .productos-main__count { font-size:0.85rem; color:#777; margin:0; }
        .productos-main__count strong { color:#1a1a1a; }
        .productos-main__clear { font-size:0.8rem; font-weight:600; color:#e05c2a; background:#fff1ec; border:none; border-radius:99px; padding:4px 12px; cursor:pointer; transition:background 0.15s; }
        ...
        @media (max-width:768px) {
          ...
          .sidebar { position:fixed; top:0; left:0; width:min(320px,85vw); height:100dvh; overflow-y:auto; z-index:200; border-radius:0; transform:translateX(-100%); transition:transform 0.28s cubic-bezier(.4,0,.2,1); box-shadow:4px 0 24px rgba(0,0,0,0.12); }
