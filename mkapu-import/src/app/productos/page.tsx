...
const ALL_CATS = Array.from(new Set(productsData.map((p) => p.category)));

const allPrices = productsData.map((p) => p.price).filter((p) => p > 0);
const PRICE_MIN = 0;
const PRICE_MAX = allPrices.length > 0 ? Math.ceil(Math.max(...allPrices) / 100) * 100 : 5000;
...
export default function ProductosPage() {
  ...
  const activeFilters = cats.length + (onlyFeatured ? 1 : 0) + (maxPrice < PRICE_MAX ? 1 : 0);

  return (
    <div className="productos-page">

      {/* Mobile filter toggle */}
      <button className="filter-toggle" onClick={() => setSidebarOpen(true)} aria-label="Abrir filtros">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <line x1="4" y1="6" x2="20" y2="6"/><line x1="8" y1="12" x2="20" y2="12"/><line x1="12" y1="18" x2="20" y2="18"/>
        </svg>
        Filtros
        {activeFilters > 0 && <span className="filter-toggle__badge">{activeFilters}</span>}
      </button>
        ...
        <aside className={`sidebar${sidebarOpen ? " sidebar--open" : ""}`}>
          <div className="sidebar__header">
            ...
            <div className="sidebar__header-actions">
              ...
              <button className="sidebar__close" onClick={() => setSidebarOpen(false)} aria-label="Cerrar filtros">×</button>
            </div>
          </div>
          ...
          {/* Featured */}
          <div className="sidebar__section">
            <label className="sidebar__check-row sidebar__check-row--featured">
              <input
                type="checkbox"
