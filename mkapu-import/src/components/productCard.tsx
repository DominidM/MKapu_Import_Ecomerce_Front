...
export default function ProductCard({ product }: Props) {
  ...
  return (
    <article className={`pcard${added ? " pcard--pop" : ""}`}>
      ...
      <div className="pcard__price" onClick={handleCardClick} style={{cursor: 'pointer'}}>
        ...
        {qty === 0 ? (
          ...
          <div className="pcard__stepper">
            ...
            <div className="pcard__step-info">
              ...
              <span className="pcard__step-tier">S/ {activePriceCart.toFixed(2)} c/u</span>
            </div>
            <button className="pcard__step" onClick={() => handleUpdateQty(qty + 1)}>+</button>
          </div>
        )}
      ...
      <style jsx>{`
        ...
        .pcard__badge--featured { background:#10b981; color:#fff; top:38px; }

        .pcard__qty-badge { position:absolute; bottom:7px; right:9px; font-size:0.6rem; font-weight:800; border-radius:99px; padding:2px 7px; border:2px solid #fff; z-index:1; animation:bdg .2s ease; background:var(--accent); color:#fff; }
        ...
        .pcard__price-label { font-size:0.71rem; font-weight:600; color:var(--text); }
        .pcard__price-value { font-size:0.92rem; font-weight:900; color:var(--accent); }
        ...
        .pcard__step-tier { font-size:0.58rem; font-weight:600; line-height:1; color:var(--accent); }
      `}</style>
    </article>
  );
}